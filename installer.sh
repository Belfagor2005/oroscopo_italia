#!/bin/bash
## setup command=wget -q --no-check-certificate https://raw.githubusercontent.com/Belfagor2005/oroscopo_radioitalia/main/installer.sh -O - | /bin/sh

version='1.0'
changelog='\nInit Plugin'

TMPPATH=/tmp/oroscopo_radioitalia-install
FILEPATH=/tmp/oroscopo_radioitalia-main.tar.gz

echo "Starting Oroscopo RadioItalia installation..."
...
cleanup() {
    ...
    [ -d "/tmp/oroscopo_radioitalia-main" ] && rm -rf "/tmp/oroscopo_radioitalia-main"
}
...
echo "⬇️ Downloading Oroscopo RadioItalia..."
wget --no-check-certificate 'https://github.com/Belfagor2005/oroscopo_radioitalia/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download Oroscopo RadioItalia package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract Oroscopo RadioItalia package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/oroscopo_radioitalia-main/usr/lib/enigma2/python/Plugins/Extensions/oroscopo" ]; then
    cp -r "$TMPPATH/oroscopo_radioitalia-main/usr/lib/enigma2/python/Plugins/Extensions/oroscopo"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/oroscopo_radioitalia-main/usr/lib64/enigma2/python/Plugins/Extensions/oroscopo" ]; then
    cp -r "$TMPPATH/oroscopo_radioitalia-main/usr/lib64/enigma2/python/Plugins/Extensions/oroscopo"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/oroscopo_radioitalia-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/oroscopo_radioitalia-main/usr"/* /usr/ 2>/dev/null
